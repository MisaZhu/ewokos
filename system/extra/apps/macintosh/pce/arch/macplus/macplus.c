...
void mac_check_mouse (macplus_t *sim)
{
	...
	sim->mouse_delta_x = sim->mouse_target_x - sim->mouse_x;
	sim->mouse_delta_y = sim->mouse_target_y - sim->mouse_y;
	...
	if ((sim->mouse_delta_x <= -2) || (sim->mouse_delta_x >= 2)) {
		...
		sim->mouse_x = RANGE(sim->mouse_x, 0 , 512);
	...
	if ((sim->mouse_delta_y <= -2) || (sim->mouse_delta_y >= 2)) {
		...
		sim->mouse_y = RANGE(sim->mouse_y, 0 , 342);
...
static
void mac_set_mouse (void *ext, int x, int y, unsigned but)
{
	macplus_t     *sim = ext;
	unsigned char old;
	...
	sim->mouse_target_x = x;
	sim->mouse_target_y = y;
}
