...
void trm_set_size (terminal_t *trm, unsigned w, unsigned h)
{
	...
	cnt = 4 * (unsigned long) w * (unsigned long) h;
...
void trm_set_mouse_absolute (terminal_t *trm, int x, int y, unsigned but)
{
	if (trm->set_mouse != NULL) {
		trm->set_mouse (trm->set_mouse_ext, x, y, but);
	}
}
