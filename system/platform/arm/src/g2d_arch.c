/* ARMv7 NEON back end for the g2d arch hooks. The semantics mirror the
   ...
   Same structure as the aarch64 back end, adapted for ARMv7 NEON:
   - no vmaxvq/vminvq horizontal reductions (AArch64 only): pairwise
     vpmax/vpmin folding instead
   - no PRFM: __builtin_prefetch (inline pld is rejected by the assembler
     for this target)
   - no -mstrict-align here, so plain vld1q/vst1q intrinsics stay genuine
     NEON; multi-register copies still use pinned d-register asm because
     this GCC has no vld1q_u32_x4-style intrinsics */
#include <stdint.h>
#include <stdlib.h>
#include <string.h>
#include <g2d_arch.h>
...
/* Horizontal max/min via pairwise folding (ARMv7 NEON has no vmaxvq/vminvq
   — those are AArch64 only) */
static inline uint32_t neon_hmax_u32x4(uint32x4_t v)
{
    uint32x2_t m = vpmax_u32(vget_low_u32(v), vget_high_u32(v));
    m = vpmax_u32(m, m);
    return vget_lane_u32(m, 0);
}

static inline uint32_t neon_hmin_u32x4(uint32x4_t v)
{
    uint32x2_t m = vpmin_u32(vget_low_u32(v), vget_high_u32(v));
    m = vpmin_u32(m, m);
    return vget_lane_u32(m, 0);
}
...
/* Copy 16 pixels (64 bytes) with multi-register VLD1/VST1: 4 instructions
   ...
   only pass aligned pointers (see g2d_row_copy_neon). */
static inline void g2d_copy_16_neon(uint32_t *dp, const uint32_t *sp) {
    __asm__ volatile(
        "vld1.32 {d0-d3}, [%0]\n\t"
        "vld1.32 {d4-d7}, [%1]\n\t"
        "vst1.32 {d0-d3}, [%2]\n\t"
...
/* 16px sub-block already loaded interleaved (cheap vld1q). Alphas are the
   ...
   expensive vld4/vst4 de-interleave path: transparent skips all memory
   access, opaque is a plain copy. Only truly mixed blocks reload the
        ...
        uint32x4_t s0, uint32x4_t s1, uint32x4_t s2, uint32x4_t s3)
{
    ...
    if(neon_hmax_u32x4(vshrq_n_u32(or_a, 24)) == 0)
        return;
    uint32x4_t and_a = vandq_u32(vandq_u32(s0, s1), vandq_u32(s2, s3));
    if(neon_hmin_u32x4(vshrq_n_u32(and_a, 24)) == 0xff) {
        vst1q_u32(dp, s0);
        vst1q_u32(dp + 4, s1);
        vst1q_u32(dp + 8, s2);
        vst1q_u32(dp + 12, s3);
...
static inline void g2d_alpha32_a255(uint32_t *dp, const uint32_t *sp)
{
    ...
    uint32x4_t s2 = vld1q_u32(sp + 8);
    uint32x4_t s3 = vld1q_u32(sp + 12);
    uint32x4_t s4 = vld1q_u32(sp + 16);
    uint32x4_t s5 = vld1q_u32(sp + 20);
    uint32x4_t s6 = vld1q_u32(sp + 24);
    uint32x4_t s7 = vld1q_u32(sp + 28);
...
{
    g2d_alpha16_a255_regs(dp, sp,
            vld1q_u32(sp), vld1q_u32(sp + 4),
            vld1q_u32(sp + 8), vld1q_u32(sp + 12));
}
