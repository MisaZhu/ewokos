...
void graph_scale_tof_fast_bsp(graph_t* g, graph_t* dst, double scale) {
    ...
    for(int i = 0; i < dst->h; i++) {
        ...
        for(; j <= dst->w - 8; j += 8) {
            ...
            // Process 8 pixels using ARM NEON (32-bit registers)
            // ARM uses d0-d31 (64-bit) / q0-q15 (128-bit) register naming
            for(int pair = 0; pair < 2; pair++) {
                ...
                // ARM NEON intrinsics for bilinear interpolation
                ...
                // Extract color channels using ARM NEON
                ...
                vst1q_u32(&dst->buffer[i * dst->w + j + offset], result);
            }
        }
