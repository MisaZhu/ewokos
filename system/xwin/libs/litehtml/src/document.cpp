...
litehtml::document::ptr litehtml::document::createFromUTF8(const char* str, litehtml::document_container* objPainter, litehtml::context* ctx, litehtml::css* user_styles)
{
	...
	// Initialize enable_shared_from_this manually
	auto* esft = static_cast<std::enable_shared_from_this<litehtml::document>*>(doc.get());
	esft->_enable_shared_from_this(&doc);
...
litehtml::element::ptr litehtml::document::create_element(const tchar_t* tag_name, const string_map& attributes)
{
	...
	// Initialize enable_shared_from_this for element manually
	if (newTag)
	{
		auto* esft = static_cast<std::enable_shared_from_this<litehtml::element>*>(newTag.get());
		esft->_enable_shared_from_this(&newTag);
	}
	...
	case GUMBO_NODE_TEXT:
		{
			...
			for (size_t i = 0; i < str_in.length(); i++)
			{
				...
				if (c <= ' ' && (c == ' ' || c == '\t' || c == '\n' || c == '\r' || c == '\f'))
				{
					if (!str.empty())
					{
						elements.push_back(std::make_shared<el_text>(str.c_str(), shared_from_this()));
						str.clear();
					}
					str += c;
					elements.push_back(std::make_shared<el_space>(str.c_str(), shared_from_this()));
					str.clear();
				}
