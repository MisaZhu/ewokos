...
litehtml::element::ptr litehtml::elements_iterator::next(bool ret_parent)
{
	...
	while(m_el && m_idx < (int) m_el->get_children_count())
	{
...
{
	m_idx++;
	while(m_el && m_idx >= (int) m_el->get_children_count() && m_stack.size())
	{
		stack_item si = m_stack.back();
		m_stack.pop_back();
		m_idx	= si.idx;
		// Check if si.el is valid before assigning
		if (si.el)
		{
			m_el	= si.el;
		}
		...
		{
			// If si.el is invalid, break the loop
			break;
		}
		m_idx++;
		continue;
