...
static int vfs_split_fcntl(int fd, int cmd, proto_t* arg_in, proto_t* arg_out){ 
    ...
    do{
        ...
        PF->init(&out);
        vfs_fcntl(fd, SOCK_REQUEST,  &in , &out);
...
int accept (int fd, struct sockaddr* addr,uint32_t * addr_len){
    ...
    PF->init(&out);
	vfs_split_fcntl(fd, SOCK_ACCEPT, NULL , &out);
    ret = proto_read_int(&out);
    if(ret > 0){
        proto_read_to(&out, addr, addr_len);
    }
...
int shutdown (int fd, int how){
    ...
    PF->init(&out);
    vfs_fcntl(fd, SOCK_CLOSE, NULL , &out);
    ret = proto_read_int(&out);
    PF->clear(&out);
    return ret;
}
