...
void Graph::fillRound(int32_t x, int32_t y, int32_t w, int32_t h, uint32_t radius, uint32_t color) {
	if(g == NULL)
		return;
	graph_fill_round(g, x, y, w, h, radius, color);
}

void Graph::round(int32_t x, int32_t y, int32_t w, int32_t h, uint32_t radius, uint32_t color) {
	if(g == NULL)
		return;
	graph_round(g, x, y, w, h, radius, color);
}
