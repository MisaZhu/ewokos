...
3. Create or import an e2studio project that builds the wolfSSL library sources for your target board.
...
4. Create an `smc_gen` folder under your e2studio test project `src/` directory.
5. Copy the FIT modules into the folder that is created at step 4.
6. Open your e2studio example project.
7. Enable the application-type macro definitions in your project's `wolfssl_demo.h`.
...
Equivalent helper scripts can be provided from your local project tooling or the upstream Renesas integration examples.
