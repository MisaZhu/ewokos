...
our %config = (
    ...
    "openssl_sys_defines" => [],
    "openssldir" => "",
    "options" => "--prefix=/tmp/openssl no-acvp-tests no-afalgeng no-asan no-asm no-buildtest-c++ no-capieng no-crypto-mdebug no-crypto-mdebug-backtrace no-devcryptoeng no-dynamic-engine no-ec_nistp_64_gcc_128 no-egd no-engine no-external-tests no-fips no-fips-securitychecks no-fuzz-afl no-fuzz-libfuzzer no-ktls no-loadereng no-md2 no-msan no-padlockeng no-rc5 no-sctp no-shared no-ssl3 no-ssl3-method no-tests no-trace no-ubsan no-unit-test no-uplink no-weak-ssl-ciphers no-zlib no-zlib-dynamic",
    "patch" => "13",
...
unless (caller) {
    ...
    use lib '/Users/mingmingzhu/work/ewokos/system/network/libs/openssl/util/perl';
    use OpenSSL::fallback '/Users/mingmingzhu/work/ewokos/system/network/libs/openssl/external/perl/MODULES.txt';
    ...
    if (scalar @ARGV == 0) {
        ...
        use lib '.';
        use lib '/Users/mingmingzhu/work/ewokos/system/network/libs/openssl/Configurations';
        use gentemplate;
