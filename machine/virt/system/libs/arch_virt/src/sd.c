...
int virtio_blk_rw(struct virtio_device *dev, void *buffer, uint32_t count, int write) {
    ...
    while (timeout--){
        ...
        for (volatile uint64_t i = 0; i < 100; i++);
    }
...
static int plat_dma_alloc(void* vaddr, void* paddr, int size){
	uint32_t v = (uint32_t)(dma_alloc(0, size)&0xFFFFFFFF);
	uint32_t p = (uint32_t)(dma_phy_addr(0, (uint32_t)v)&0xFFFFFFFF);
	*(uint32_t*)vaddr = v;
	*(uint32_t*)paddr = p;
	//klog("DMA alloc p:%08x v:%08x size:%d\n", *paddr, *vaddr, size);
	*(char*)v = 0;
}
...
int32_t virt_sd_init(void) {
	_mmio_base = mmio_map();
    uintptr_t base = _mmio_base + 0x02003e00;
...
int32_t virt_sd_read_sector(int32_t sector, void* buf) {
	...
	return virtio_blk_rw(&device, buf, 1, 0);
}
