...
void dump_page_tables(page_dir_entry_t *vm){
	...
	for(uint64_t i = 0; i < 4; i++){
		if(vm[i].NSTable){
			...
			for(uint64_t j = 0; j < PAGE_DIR_NUM; j++){
				if(l2_table[j].EntryType == TYPE_BLOCK){
					...
					for(uint64_t k = 0; k < PAGE_DIR_NUM; k++)
						if((uint64_t)l3_table[k].AF){
							printf("\t\t%08llx:%016llx\n",
									(unsigned long long)((i << PAGE_L1_SHIFT) + (j << PAGE_L2_SHIFT) + (k << PAGE_L3_SHIFT)),
...
inline void clear_cache(void *start, void *end) {
    ...
    // Align to the cache line size, typically 64 bytes.
    addr &= ~(64 - 1);

    // Flush the data cache with DC CIVAC.
