...
void gic_v3_irq_enable(int core, int irq){
	...
	gicd_set_priority(irq, 0 << GIC_PRI_SHIFT );  /* Set priority */
	gicd_set_target(irq, core);  /* processor 0 */
	gicd_clear_pending(irq);
	gicd_enable_int(irq);
}
...
int gic_v3_get_irq(void){
	uint64_t irq;
	//__asm__ __volatile__("mrs %[v], ICC_IAR1_EL1" : [v]"=r" (irq)::);
	//__asm__ __volatile__("msr ICC_EOIR1_EL1, %[v]" : : [v]"r"(irq):);

	irq = (*REG_GIC_GICC_IAR) & GICC_IAR_INTR_IDMASK;
    gic_v3_eoi(irq);
    return irq;
}
