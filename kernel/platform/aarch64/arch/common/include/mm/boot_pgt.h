...
static inline page_table_entry_t* boot_pgt_get_next_table(boot_pgt_ctx_t* ctx, page_dir_entry_t* entry) {
	...
	if(entry->EntryType == 0) {
		...
		*entry = (page_dir_entry_t){
		...
		pte_set_address((page_table_entry_t*)entry, (uint64_t)table >> PAGE_SHIFT);
	}
	else {
		table = (page_table_entry_t*)((uint64_t)pte_get_address((page_table_entry_t*)entry) << PAGE_SHIFT);
	}
...
static inline void boot_pgt_map_range(boot_pgt_ctx_t* ctx, uint64_t virt, uint64_t phy, uint32_t len, int is_dev) {
	...
	while(virt < end) {
		uint32_t root = PAGE_ROOT_INDEX(virt);
		uint32_t l3 = PAGE_L3_INDEX(virt);
		page_table_entry_t* l3_table = boot_pgt_get_next_table(ctx, &ctx->page_dir[root]);

		pte_set_address(&l3_table[l3], phy >> PAGE_SHIFT);
	...
	while(virt < end) {
		...
		pte_set_address(&l2_table[l2], phy >> PAGE_SHIFT);
