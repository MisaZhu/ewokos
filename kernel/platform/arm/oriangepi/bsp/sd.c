...
struct sunxi_mmc {
    volatile uint32_t gctrl;      /* 0x00 global control */
    volatile uint32_t clkcr;      /* 0x04 clock control */
    volatile uint32_t timeout;        /* 0x08 time out */
    volatile uint32_t width;      /* 0x0c bus width */
    volatile uint32_t blksz;      /* 0x10 block size */
    volatile uint32_t bytecnt;        /* 0x14 byte count */
    volatile uint32_t cmd;        /* 0x18 command */
    volatile uint32_t arg;        /* 0x1c argument */
    ...
    volatile uint32_t imask;      /* 0x30 interrupt mask */
    volatile uint32_t mint;       /* 0x34 masked interrupt status */
    volatile uint32_t rint;       /* 0x38 raw interrupt status */
    volatile uint32_t status;     /* 0x3c status */
    volatile uint32_t ftrglevel;      /* 0x40 FIFO threshold watermark*/
    volatile uint32_t funcsel;        /* 0x44 function select */
    volatile uint32_t cbcr;       /* 0x48 CIU byte count */
    volatile uint32_t bbcr;       /* 0x4c BIU byte count */
    volatile uint32_t dbgc;       /* 0x50 debug enable */
    volatile uint32_t csdc;       /* 0x54 CRC status detect control register */
    volatile uint32_t a12a;       /* 0x58 Auto command 12 argument */
    volatile uint32_t ntsr;       /* 0x5c New timing set register */
    volatile uint32_t res1[6];
    volatile uint32_t hwrst;      /* 0x78 SMC eMMC Hardware Reset Register */
    volatile uint32_t res6;       /* 0x7c */
    volatile uint32_t dmac;       /* 0x80 internal DMA control */
    volatile uint32_t dlba;       /* 0x84 internal DMA descr list base address */
    volatile uint32_t idst;       /* 0x88 internal DMA status */
    volatile uint32_t idie;       /* 0x8c internal DMA interrupt enable */
    volatile uint32_t chda;       /* 0x90 */
    volatile uint32_t cbda;       /* 0x94 */
    volatile uint32_t res2[90];
    volatile uint32_t fifo;       /* 0x100 / 0x200 FIFO access address */
};
...
int32_t sd_dev_read(int32_t sector) {
    ...
    data.dest = (char*)_sector_buf;
    data.blocks = 1;
    data.blocksize = 512;
    data.flags = MMC_DATA_READ;

    if (send_command((void*)(_sys_info.mmio.v_base + 0x4020000L), &cmd, &data)){
        return -1;
    }
