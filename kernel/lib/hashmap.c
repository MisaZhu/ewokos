...
map_t hashmap_new() {
	...
	m->data = (hashmap_element*) kcalloc(INITIAL_SIZE, sizeof(hashmap_element));
	if(!m->data) goto err;
...
int hashmap_rehash(map_t in){
	...
	hashmap_element* temp = (hashmap_element *)
		kcalloc(2 * m->table_size, sizeof(hashmap_element));
...
int hashmap_put(map_t in, const char* key, any_t value){
	...
	m->data[index].key = (char*)kmalloc(strlen(key)+1);
