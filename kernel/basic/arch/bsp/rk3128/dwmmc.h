...
#define DWMCI_IDINTEN_RI		0x2
#define DWMCI_IDINTEN_TI		0x1
...
struct dwmci_host {
	 ...
	 */
	unsigned int (*get_mmc_clk)(struct dwmci_host *host, uint32_t freq);
...
static inline void dwmci_writew(struct dwmci_host *host, int reg, uint16_t val)
{
	writew(val, host->ioaddr + reg);
}

static inline void dwmci_writeb(struct dwmci_host *host, int reg, uint8_t val)
{
...
static inline uint16_t dwmci_readw(struct dwmci_host *host, int reg)
{
	return readw(host->ioaddr + reg);
}

static inline uint8_t dwmci_readb(struct dwmci_host *host, int reg)
{
...
void dwmci_setup_cfg(struct mmc_config *cfg, struct dwmci_host *host,
		uint32_t max_clk, uint32_t min_clk);
 ...
 */
int add_dwmci(struct dwmci_host *host, uint32_t max_clk, uint32_t min_clk);
#endif /* !CONFIG_BLK */
