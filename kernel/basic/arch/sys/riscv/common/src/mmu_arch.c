...
int32_t map_page(page_dir_entry_t *vm, uint32_t virtual_addr,
	...
	if (vm[ppn2].ppn == 0) {
		...
		level1 = BASE_TO_PAGE_TABLE(vm[ppn2].ppn);
	}

	/* if this page_dirEntry is not mapped before, map it to a new page table */
	if (level1[ppn1].ppn == 0) {
		level0 = kalloc4k();
		if(level0 == NULL)
			return -1;
		...
		level0 = BASE_TO_PAGE_TABLE(level1[ppn1].ppn);
	}

	/* map the virtual page to physical page in page table */
	level0[ppn0].valid = 1;
	level0[ppn0].access = 1;
	level0[ppn0].ppn = PAGE_TABLE_TO_BASE(V2P(physical)); 
	set_pte_permission(&level0[ppn0], permissions);
	set_pte_flags(&level0[ppn0], pte_attr);
	
	//printf("table:%08x mape %08x -> %08x, permission:%d\n", vm, physical, virtual_addr, permissions);
	return 0;
}
...
page_table_entry_t* get_page_table_entry(page_dir_entry_t *vm, uint32_t virtual) {
	...
	if(vm[ppn2].valid){
		level1 = BASE_TO_PAGE_TABLE(vm[ppn2].ppn);
		if(level1[ppn1].valid){
			return BASE_TO_PAGE_TABLE(level1[ppn1].ppn);
		}
	}
	return NULL;
}
