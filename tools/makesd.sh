...
echo ""
echo "bootfs partition: ${PART_BOOT}"
echo "rootfs partition: ${PART_ROOT}"
...
sudo newfs_msdos -F 32 -v bootfs "${PART_BOOT}"

echo "[4] Re‑format ${PART_ROOT} as ext3 (-F force skip confirmation prompt)"
sudo "${MKE2FS}" -F -t ext3 -L rootfs -b 4096 -I 128 "${PART_ROOT}"
