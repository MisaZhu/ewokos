...
inline uint32_t vfsAdd(uint32_t node, const char* name, uint32_t size) {
	...
	PackageT* pkg = ipcReq(servPid, 0, VFS_CMD_ADD, proto->data, proto->size, true);
	protoFree(proto);
...
inline int32_t vfsDel(uint32_t node) {
	...
	PackageT* pkg = ipcReq(servPid, 0, VFS_CMD_DEL, &node, 4, true);
	if(pkg == NULL || pkg->type == PKG_TYPE_ERR) {
		if(pkg != NULL) free(pkg);
		return -1;
	}
...
inline int32_t vfsNodeInfo(uint32_t node, FSInfoT* info) {
	...
	PackageT* pkg = ipcReq(servPid, 0, VFS_CMD_INFO, &node, 4, true);
	if(pkg == NULL || pkg->type == PKG_TYPE_ERR) {
		if(pkg != NULL) free(pkg);
		return -1;
	}
...
inline uint32_t vfsNodeByFD(int32_t fd) {
	...
	PackageT* pkg = ipcReq(servPid, 0, VFS_CMD_NODE_BY_FD, &fd, 4, true);
	if(pkg == NULL || pkg->type == PKG_TYPE_ERR) {
		if(pkg != NULL) free(pkg);
		return -1;
	}
...
inline uint32_t vfsNodeByName(const char* fname) {
	...
	PackageT* pkg = ipcReq(servPid, 0, VFS_CMD_NODE_BY_NAME, proto->data, proto->size, true);
	protoFree(proto);
	if(pkg == NULL || pkg->type == PKG_TYPE_ERR) {
		if(pkg != NULL) free(pkg);
		return 0;
...
inline uint32_t vfsMount(const char* fname, const char* devName, int32_t devIndex, bool isFile) {
	...
	PackageT* pkg = ipcReq(servPid, 0, VFS_CMD_MOUNT, proto->data, proto->size, true);
	protoFree(proto);
	if(pkg == NULL || pkg->type == PKG_TYPE_ERR) {
		if(pkg != NULL) free(pkg);
		return 0;
...
inline int32_t vfsUnmount(uint32_t node) {
	...
	PackageT* pkg = ipcReq(servPid, 0, VFS_CMD_UNMOUNT, &node, 4, true);
	if(pkg == NULL || pkg->type == PKG_TYPE_ERR) {
		if(pkg != NULL) free(pkg);
		return -1;
	}
...
FSInfoT* vfsKids(uint32_t node, uint32_t* num) {
	*num = 0;
	...
	PackageT* pkg = ipcReq(servPid, 0, VFS_CMD_KIDS, &node, 4, true);
