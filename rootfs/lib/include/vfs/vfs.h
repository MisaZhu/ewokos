...
int32_t vfs_add(const char* dir_name, const char* name, uint32_t size, void* data);
...
int32_t vfs_info_by_name(const char* fname, fs_info_t* info);

int32_t vfs_info_by_fd(int32_t fd, fs_info_t* info);

int32_t vfs_node_update(fs_info_t* info);

int32_t vfs_pipe_open(int32_t fds[2]);

int32_t vfs_mount(const char* fname, const char* devName, int32_t devIndex, bool isFile);

int32_t vfs_unmount(const char* fname);
