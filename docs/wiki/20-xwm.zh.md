...
> 建议先读完 [第 09 章 IPC](09-ipc.zh.md) 和
> [第 13 章 图形系统](13-graphics.zh.md)。
...
本章只把 xserverd 当作"调用 xwm 的那一方"。它自己怎样接收客户端的帧、
判断哪些窗口要重画、把 `frame_g` 和客户端缓冲拼成一帧并只推脏矩形——
请继续读 [第 22 章 xserverd 合成器：绘制流程与脏区机制](22-xserverd-render.zh.md)。
